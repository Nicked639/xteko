  ...
  views: [{
    ...
    events: {
      changed(sender) {
        ...
        $("initialView").contentOffset = $point(0, 0);
        $("loading").text = "Loading..."
        let search = clipboardDetect().keyword
        let c = $clipboard.text
        if (sender.index == 2) {
          if(search) $clipboard.text = "";
